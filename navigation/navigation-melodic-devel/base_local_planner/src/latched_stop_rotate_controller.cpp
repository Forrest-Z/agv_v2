...
bool LatchedStopRotateController::isGoalReached(LocalPlannerUtil* planner_util, OdometryHelperRos& odom_helper, const geometry_msgs::PoseStamped& global_pose) {
  ...
  // ROS_INFO("latched_stop_rotate_controller.cpp-73-xy_goal_tolerance: %f", xy_goal_tolerance);
  // ROS_INFO("latched_stop_rotate_controller.cpp-74-theta_stopped_vel: %f", theta_stopped_vel);
  // ROS_INFO("latched_stop_rotate_controller.cpp-75-trans_stopped_vel: %f", trans_stopped_vel);
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    if (latch_xy_goal_tolerance_ && ! xy_tolerance_latch_) {
      ROS_INFO("latched_stop_rotate_controller.cpp-98-Goal position reached (check), stopping and turning in place");
      xy_tolerance_latch_ = true;
    }
...
bool LatchedStopRotateController::stopWithAccLimits(const geometry_msgs::PoseStamped& global_pose,
  ...
  if(valid_cmd){
    ROS_INFO("latched_stop_rotate_controller.cpp-139-Slowing down... using vx, vy, vth: %.2f, %.2f, %.2f", vx, vy, vth);
    cmd_vel.linear.x = vx;
    cmd_vel.linear.y = vy;
    cmd_vel.angular.z = vth;
    return true;
  }
  ROS_WARN("latched_stop_rotate_controller.cpp-146-Stopping cmd in collision");
...
bool LatchedStopRotateController::rotateToGoal(
  ...
  ROS_WARN("latched_stop_rotate_controller.cpp-198-Rotation cmd in collision");
  cmd_vel.angular.z = 0.0;
  return false;
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    if ( ! rotating_to_goal_ && !base_local_planner::stopped(base_odom, limits.theta_stopped_vel, limits.trans_stopped_vel)) {
      if ( ! stopWithAccLimits(
          ...
          sim_period,
          obstacle_check)) {
        ROS_ERROR("latched_stop_rotate_controller.cpp-253-Error when stopping.");
        return false;
      }
      ROS_INFO("latched_stop_rotate_controller.cpp-256-Stopping...");
    }
